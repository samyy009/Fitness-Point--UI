...
export default function Home() {
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      <section className="section" id="programs" style={{ background: 'var(--dark-2)' }}>
        <div className="container">
          <motion.div className="section-header" {...fadeUp()}>
            <div className="section-tag">Explore Programs</div>
            <h2 className="section-title">Explore Our Fitness Programs</h2>
            <p className="section-subtitle">Choose from a variety of training categories designed to suit your fitness level.</p>
          </motion.div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '2rem' }}>
            {programs.map((p, idx) => (
              ...
              >
                <div>
                  <div style={{ fontSize: '2.5rem', marginBottom: '1.25rem', background: 'rgba(0,229,255,0.06)', width: '64px', height: '64px', borderRadius: 'var(--radius)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    {p.icon}
                  </div>
                  <h3 style={{ fontSize: '1.3rem', fontWeight: 800, marginBottom: '0.75rem' }}>{p.title}</h3>
                  <p style={{ fontSize: '0.9rem', color: 'var(--text-light)', lineHeight: '1.6', marginBottom: '1.5rem' }}>{p.desc}</p>
                </div>
                <button 
      ...
      <section className="section" id="pricing" style={{ background: 'var(--dark)' }}>
        <div className="container">
          <motion.div className="section-header" {...fadeUp()}>
            <div className="section-tag">Pricing Options</div>
            <h2 className="section-title">Choose Your Perfect Plan</h2>
            <p className="section-subtitle">Flexible pricing options designed to fit your goals and budget.</p>
          </motion.div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '2rem', alignItems: 'stretch' }}>
            {plans.map((p, idx) => (
              <motion.div
                key={idx}
                className={`card-premium ${p.popular ? 'glow-border' : ''}`}
                {...fadeUp(idx * 0.1)}
                whileHover={{ scale: 1.02 }}
                style={{ 
                  display: 'flex', 
              ...
              >
                {p.popular && (
                  <span className="badge badge-primary" style={{ position: 'absolute', top: '1.25rem', right: '1.25rem', fontSize: '0.65rem' }}>
                    Recommended
                  </span>
                )}
                <div>
                  <h3 style={{ fontSize: '1.25rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.5rem' }}>{p.name}</h3>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.25rem', margin: '1.5rem 0' }}>
                    <span style={{ fontSize: '2.5rem', fontWeight: 900, color: 'var(--primary)' }}>{p.price}</span>
                    <span style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>/month</span>
                  </div>
                  <ul style={{ display: 'grid', gap: '0.85rem', marginBottom: '2.5rem' }}>
                    {p.features.map((feat, i) => (
                      <li key={i} style={{ fontSize: '0.9rem', color: 'var(--text-light)', display: 'flex', gap: '0.6rem', alignItems: 'center' }}>
                        <span style={{ color: 'var(--primary)', fontWeight: 'bold' }}>✓</span> {feat}
                      </li>
                    ))}
                  </ul>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                  <button 
                    className={`btn ${p.popular ? 'btn-primary btn-shine' : 'btn-ghost'} btn-lg`}
                  ...
                  <button 
                    ...
                    style={{ width: '100%', justifyContent: 'center', borderColor: 'rgba(255,255,255,0.1)', color: 'var(--text-light)' }}
      ...
      <section className="section" id="trainers" style={{ background: 'var(--dark-2)' }}>
        <div className="container">
          <motion.div className="section-header" {...fadeUp()}>
            <div className="section-tag">Elite Mentors</div>
            <h2 className="section-title">Meet Our Certified Trainers</h2>
            <p className="section-subtitle">Our team of professional trainers is here to guide, motivate, and help you reach your goals.</p>
          </motion.div>

          <div className="trainers-grid">
            {trainers.map((m, idx) => (
              <motion.div key={idx} className="trainer-card" {...fadeUp(idx * 0.1)}>
                <div className="trainer-avatar" style={{ overflow: 'hidden', height: '260px' }}>
                  <img 
                    src={m.img} 
                    alt={m.name} 
                    loading="lazy" 
                ...
                <h4 className="trainer-name" style={{ fontSize: '1.15rem', fontWeight: 800, marginTop: '1.25rem' }}>{m.name}</h4>
                <p className="trainer-specialty" style={{ color: 'var(--primary)', fontWeight: 600, fontSize: '0.85rem' }}>{m.role}</p>
                <div style={{ marginTop: '0.5rem', display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                  <span style={{ fontSize: '0.8rem', color: 'var(--text-light)' }}><strong>Certs:</strong> {m.certs}</span>
                  <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>{m.exp}</span>
                </div>
                <button
                  className="btn btn-outline btn-sm"
                  style={{ marginTop: '1.25rem', width: '100%', justifyContent: 'center' }}
                  onClick={() => setBookingModalTrainer(m)}
                >
                  Book Trainer
                </button>
              </motion.div>
